...
These lines divide the phase plane into different regions where the signs of $\frac{dR}{dt}$ and $\frac{dE}{dt}$ change. The nullclines intersect at the equilibrium points $(0,0)$ and $(1.111, 1.818)$.

From part (b), we found that the origin is a saddle point, so trajectories move away from it in one direction and toward it in another. The interior equilibrium is a center, which means trajectories form closed loops around it.

Therefore, if we start at the initial condition, the system will move in a closed orbit around the interior equilibrium. This results in oscillations of both the resource and enzyme concentrations. Biologically, this means that the resource and enzyme levels repeatedly increase and decrease instead of settling at a fixed value.
